...
// typewriter effect fo the joke
function textTypingEffect(element, joke, i = 0) {
    if (i === 0) {
        element.textContent = "";
    }
    textDiv.hidden = false;
    element.textContent += joke[i];
    // if we reached the end of the string stp
    if (i === joke.length - 1) {
        return;
    }
    setTimeout(() => textTypingEffect(element, joke, i + 1), 50)
}
...
// Passing the joke to voice RSSApi
